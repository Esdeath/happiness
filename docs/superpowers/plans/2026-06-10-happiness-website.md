...
- [ ] **Step 6: 创建 `src/styles/global.css`（设计令牌 + 中文排版基线）**
...
- [ ] **Step 7: 创建临时首页 `src/pages/index.astro`**
...
- [ ] **Step 2: 创建示例文章 `src/content/posts/what-is-happiness.md`**
...
- [ ] **Step 3: 创建示例访谈 `src/content/posts/interview-on-flow.md`**
...
- [ ] **Step 4: 创建草稿示例 `src/content/posts/draft-example.md`**
...
- [ ] **Step 5: 创建占位封面 `src/content/books/covers/flow.svg`**
...
- [ ] **Step 6: 创建占位封面 `src/content/books/covers/happier.svg`**
...
- [ ] **Step 7: 创建示例书籍 `src/content/books/flow.md`**
...
- [ ] **Step 8: 创建示例书籍 `src/content/books/happier.md`**
...
- [ ] **Step 2: 写失败的测试 `src/lib/content.test.ts`**
...
- [ ] **Step 1: 创建 `src/components/TagList.astro`**
...
- [ ] **Step 3: 创建 `src/components/BookCard.astro`**
...
- [ ] **Step 2: 创建 `src/pages/posts/index.astro`**
...
- [ ] **Step 3: 创建 `src/pages/posts/[slug].astro`**
...
- [ ] **Step 1: 创建 `src/components/PurchaseLinks.astro`**
...
- [ ] **Step 2: 创建 `src/pages/books/index.astro`**
...
- [ ] **Step 3: 创建 `src/pages/books/[slug].astro`**
...
Run: `npm run build`
Expected: 成功。`dist/tags/` 下生成 心理学/哲学/访谈/生活方式 四个标签目录（中文目录名经 URL 编码属正常）；「心理学」标签页同时包含文章与书籍
...
- [ ] **Step 2: 完整实现 `src/components/Giscus.astro`**
...
- [ ] **Step 1: 修改 `package.json` 的 build 脚本**

```json
"build": "astro build && pagefind --site dist"
```

- [ ] **Step 2: 创建 `src/pages/search.astro`**
...
Expected: 测试 5 个全绿；构建成功含 Pagefind 索引
...
- [ ] 首页：理念一句话 + 最新 5 篇文章 + 最新 4 本书（当前种子内容不足 5/4，显示全部即符合预期）
- [ ] /posts 时间倒序；draft 不出现
- [ ] 访谈页顶部渲染受访者介绍块；列表卡片带"访谈"角标
- [ ] /books 卡片含封面+一句话推荐语；详情页购买按钮 `rel="nofollow sponsored"` 新窗口
- [ ] /tags/[tag] 同时聚合文章与书
- [ ] /rss.xml 仅含 posts
- [ ] /search 在 build+preview 下可搜中文
- [ ] giscus 未配置时不渲染、不报错
- [ ] 自定义 404
- [ ] CI：npm test + npm run build
- [ ] 移动端可读（max-width 内容列、卡片在小屏折行）
